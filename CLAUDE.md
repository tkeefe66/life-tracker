...
**Dates are the third non-metric series** (2026-07-30 spec). A date is a
`calendar_events` row flagged by `COALESCE(user_is_date, is_date)`:
`is_date` is set at scan time by `metrics.title_is_date` — a word-boundary
regex on the literal word "date", deliberately NOT an AI call (user's
explicit choice) — and `user_is_date` is the user's override via the event
form's Date checkbox. Resolved dates are **excluded from the social count
and social spend** (the social floor means non-date social) but still show
on their day with a "date" chip, and appear as their own `date` kind in
every spend view plus a Dates panel on Insights (count trend, total/avg
spend, top places). Not in `METRICS` ⇒ absent from the Telegram push and
the reflection prompt's content by construction (counts/spend numbers flow
like rides; titles and venues never do — locked by tests in
`test_weekly_push.py` / `test_api_routes.py`).
...
| `calendar_events` | unique `gcal_event_id` | `user_title` / `user_is_social` overrides, `source` (`gcal`\|`manual`), `amount`, `user_removed` ("didn't happen" — separate from `user_is_social`, excluded from example feed), `recurring_event_id` (Google series id — gates classifier generalization), `confidence` (< 0.7 unresolved ⇒ `uncertain` chip). Date/place columns (2026-07-30): `is_date` / `location` are GCAL-OWNED (upsert overwrites, `is_date` derived from the title rule), `user_is_date` / `user_location` are USER columns the scan never touches; resolved as `COALESCE(user_is_date, is_date)` and `COALESCE(NULLIF(user_location,''), location)`. This override pair has NO example-feed loop — detection is a regex, there is no classifier to teach. Manual events use id `manual:<uuid4>` and write user assertions to `user_is_date`/`location` |
